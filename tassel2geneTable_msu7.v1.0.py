...
#across Xoo variants
#To be upload as publicly available genomic resource on github.com
...
THRESH = ['BOOT', 'FDR'][1] #set threshold method: bootstrap vs. false discovery rate
reduce_table = 1 # choose 1 for table reduction (sign results nly)
PREFIX = 'msuSimplified_' #output files prefix
file = 'MLM_tassle_BLB_all.txt' #tassel output file e.g. 'MLM_tassle_BLB_all.txt'
GENES_DF = 'msu_ver7_simple.csv' #ref genome gene location info
...
def restLoci(data, all_sign_locs, txt, txt2, thrs, chroms):
    ...
    for chrom in chroms:
        other_inds = rest.index[(rest.logP > thrs[int(chrom) - 1]) & (rest.Chr == chrom)]
